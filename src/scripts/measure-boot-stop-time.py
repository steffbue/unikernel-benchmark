...
def measure_boot_stop_time(ec2_client, ec2_resource, instance_id, control_instance_id):
    ...
    for i in range(NUMBER_ITERATIONS):
        ...
        while(True):
            ...
            if res_result.status_code == 200:
                ...
                results_boot.append(res_result.json()['BootTime'])
                results_stop.append(res_result.json()['StopTime'])
                break
            else:
                sleep_time = int(random.uniform(5,20))
                time.sleep(sleep_time)
    ...
    return results_boot, results_stop
...
def benchmark_linux(ec2_client, ec2_resource):
    ...
    results_boot_time, results_stop_time = measure_boot_stop_time(ec2_client, ec2_resource, linux_instance_id, control_linux_instance_id)
    results_path = '/usr/src/results'
    mh.store_results(os.path.join(results_path, 'linux-boot-times.txt'), results_boot_time)
    mh.store_results(os.path.join(results_path, 'linux-stop-times.txt'), results_stop_time)
    return
...
def benchmark_osv(ec2_client, ec2_resource):
    ...
    results_boot_time, results_stop_time = measure_boot_stop_time(ec2_client, ec2_resource, osv_instance_id, control_osv_instance_id)
    results_path = '/usr/src/results'
    mh.store_results(os.path.join(results_path, 'osv-boot-times.txt'), results_boot_time)
    mh.store_results(os.path.join(results_path, 'osv-stop-times.txt'), results_stop_time)
    return
try:
