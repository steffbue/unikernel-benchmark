...
s3_resource_t1 = boto3.session.Session().resource('s3')
s3_resource_t2 = boto3.session.Session().resource('s3')

def benchmark_linux(ec2_client, ec2_resource, s3_client, s3_resource):
    ...
    results_execution_time = measure_network_execution_time(ec2_client, ec2_resource, s3_client, s3_resource, linux_instance_id, 'linuxbenchmark')
...
def benchmark_osv(ec2_client, ec2_resource, s3_client, s3_resource):
    ...
    results_execution_time = measure_network_execution_time(ec2_client, ec2_resource, s3_client, s3_resource, osv_instance_id, 'osvbenchmark')
    mh.store_results(os.path.join(results_path, 'osv-network-execution-times.txt'), results_execution_time)
    return


try:
    t1 = threading.Thread(target=benchmark_linux,args=(ec2_client_t1, ec2_resource_t1, s3_client_t1, s3_resource_t1))
    t2 = threading.Thread(target=benchmark_osv, args=(ec2_client_t2, ec2_resource_t2, s3_client_t2, s3_resource_t2))
